...
#define DURATION_BASE  562
#define DURATION_DELTA 300
...
int irda_nec_read(int *address, int *data)
{
	...
	while (1)
	{
		...
		{
			/* 解析数据 */
			/* 1. 判断是否为9MS的低脉冲 (event.pol == 0) && */
			if (duration_in_margin(event.duration, DURATION_HEAD_LOW))
			{
				/* 进行后续判断 */
				/* 2. 读下一个高脉冲数据 */
				if ( ir_event_get_timeout(&event) == 0)
				{
					/* 3. 判断它是否4.5ms的高脉冲
					 *    或者 2.25ms的高脉冲
					 */
					if (duration_in_margin(event.duration, DURATION_HEAD_HIGH))
					{
						/* 4.5ms的高脉冲 */
						/* 4. 重复解析32位数据 */
						for (i = 0; i < 32; i++)
						{
							...
							if (ir_event_get_timeout(&event) == 0)
							{
								if (duration_in_margin(event.duration, DURATION_DATA_LOW))
								{
									/* 6. 读下一个数据, 判断它是 0.56ms/1.68ms的高脉冲 */
									if (ir_event_get_timeout(&event) == 0)
									{
										if (duration_in_margin(event.duration, DURATION_DATA1_HIGH))
										{
											/* 得到了bit 1 */
											val |= (1<<i);
										}
										else if (duration_in_margin(event.duration, DURATION_DATA0_HIGH))
										{
											/* 得到了bit 0 */
										}
										else
								...
								{
								    //printf("err:%s %dus | ",event.pol?"high":"low",event.duration);
									//printf("%s %d\n\r", __FUNCTION__, __LINE__);
									return -1;
								}
							}
					...
					else if (duration_in_margin(event.duration, DURATION_REPEAT_HIGH))
					{
						/* 2.25ms的高脉冲 */
						//printf("%s %d\n\r", __FUNCTION__, __LINE__);
						return 1;  /* 重复码 */
