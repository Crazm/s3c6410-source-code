...
void s3c6410_lcd_pin_init(void)
{
    ...
    /* 其他引脚初始化 */
    GPFCON_REG |= (0x2<<30);    /* xpwmTOUT1初始化 */
    GPNCON_REG |= (0x2<<20);    /* EINT10初始化 */
}
...
void s3c6410_lcd_controller_init(p_lcd_params plcdparams)
{
     ...
     */
    int clkval = (double)HCLK / plcdparams->time_seq.vclk -1 ;
    VIDCON0_REG = (0x0 << 17)  | (clkval << 6);
     ...
     */
     VIDTCON1_REG = ((plcdparams->time_seq.thb -1 )<< 16) |      /* 左边黑框, Horizontal Back porch */
                    ...
                    ((plcdparams->time_seq.thb -1 )<< 0) ;       /* hsync脉冲宽度 */ 
