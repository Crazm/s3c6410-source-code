...
#include "s3c6410_lcd_controller.h"
#include "../s3c6410.h"
#include "../my_printf.h"
...
void s3c6410_lcd_controller_init(p_lcd_params plcdparams)
{
     ...
     */
    int clkval = (float)HCLK / plcdparams->time_seq.vclk -1 ;
     ...
     */
     ...
     WINCON0_REG = (0x1 << 17) | (bppmode <<2) | (0x1 << 0);
...
void s3c6410_lcd_controller_init_palette(void)
{
     s3c6410_lcd_controller_disable();
     
     int i;
     volatile unsigned int *p = (volatile unsigned int *)0x77100400;
     
     WPALCON_REG |= (1<<9);   // 允许CPU修改调色板 
     WPALCON_REG &= ~(0x7);   
     WPALCON_REG |= 1;            // 调色板的数据格式: 24-bit ( 8:8:8 ) 
     for(i = 0; i<256; i++)
        {
            *p++ = i;
        }
     
     WPALCON_REG &= ~(1<<9);   // 禁止CPU访问调色板
     
     s3c6410_lcd_controller_enable();
}
...
struct lcd_controller s3c6410_lcd_controller = {
    ...
    .init_palette = s3c6410_lcd_controller_init_palette,
